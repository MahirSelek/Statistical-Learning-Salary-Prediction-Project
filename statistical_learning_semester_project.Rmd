...
stats_salary <- (subset(stats_salary, select = -c(X)))
...
plot(subset(stats_salary,select = c(salary17_18,PPG,MPG,TOPG,RPG,SPG,APG,BPG,Age)))


cor.mat=cor(subset(stats_salary,select = c(salary17_18,PPG,MPG,TOPG,RPG,SPG,APG,BPG,Age)));
...
stats_salary_cor <- 
  stats_salary %>% 
  dplyr::select(salary17_18, PPG, MPG, TOPG, RPG, SPG, APG, BPG, Age)
...
boxplot(stats_salary$PPG,main = "PPG", col='lightsalmon1')
boxplot(stats_salary$MPG,main = "MPG", col='lightsalmon1')
boxplot(stats_salary$TOPG,main = "TOPG", col='lightsalmon1')
boxplot(stats_salary$RPG,main = "RPG", col='lightsalmon1')
boxplot(stats_salary$SPG,main = "SPG", col='lightsalmon1')
boxplot(stats_salary$APG,main = "APG", col='lightsalmon1')
boxplot(stats_salary$BPG,main = "BPG", col='lightsalmon1')
boxplot(stats_salary$Age,main = "Age", col='lightsalmon1')
...
### TEST 1
# player attributes 
lm.model1 <- lm(formula= salary17_18 ~ MPG+PPG+APG+RPG+TOPG+BPG+SPG+Age, data=stats_salary) 
summary(lm.model1) #Adjusted R-squared:  0.5622

# visualize model results
par(mfrow=c(2,2))
plot(lm.model1)
mtext("Model 1: MPG+PPG+APG+RPG+TOPG+BPG+SPG+Age", side = 3, line = -28, outer = TRUE)
par(mfrow=c(1,1))
```
...
1.Convert response variable to log

```{r}
stats_salary$salary17_18_log <- log(stats_salary$salary17_18)
lm.model_log <- lm(formula= salary17_18_log ~  MPG+PPG+APG+RPG+TOPG+BPG+SPG+Age, data=stats_salary)
summary(lm.model_log) #Adjusted R-squared:  0.4548  
...
Convert response variable to sqrt

```{r}
stats_salary$salary17_18_sqrt <- sqrt(stats_salary$salary17_18)
lm.model_sqrt <- lm(formula= salary17_18_sqrt ~  MPG+PPG+APG+RPG+TOPG+BPG+SPG+Age, data=stats_salary)
summary(lm.model_sqrt) #Adjusted R-squared:  0.5692   
...
TEST 2 removing SPG predictor

```{r}
lm.model_sqrt <- lm(formula= salary17_18_sqrt ~  MPG+PPG+APG+RPG+TOPG+BPG+Age, data=stats_salary)
summary(lm.model_sqrt) #Adjusted R-squared:  0.5696   

par(mfrow=c(2,2))
plot(lm.model_sqrt)
mtext("Model 1: MPG+PPG+APG+RPG+TOPG+BPG+Age", side = 3, line = -28, outer = TRUE)
par(mfrow=c(1,1))
```

TEST 3 removing BPG predictor to obtaining BEST MODEL

```{r}
lm.model_sqrt <- lm(formula= salary17_18_sqrt ~  MPG+PPG+APG+RPG+TOPG+Age, data=stats_salary)
summary(lm.model_sqrt) #Adjusted R-squared:  0.5692

par(mfrow=c(2,2))
plot(lm.model_sqrt)
mtext("Model 1: MPG+PPG+APG+RPG+TOPG+Age", side = 3, line = -28, outer = TRUE)
par(mfrow=c(1,1))
```
...
hist(testing$salary17_18_sqrt_predicted, freq = FALSE, col=(rgb(255,0,0, max = 255, alpha = 100)), main="Predicted vs Salary value")

hist(testing$salary17_18_sqrt,  freq = FALSE,add=T, col=(rgb(0,0,255, max = 255, alpha = 100)))
...
#rose are the predicted salary values and the blue are our true salary values
